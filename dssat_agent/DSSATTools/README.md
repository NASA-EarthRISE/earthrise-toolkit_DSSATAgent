...
- The `DSSAT` class still represents the simulation environment. It accepts one optional parameter when initialized: the `run_path` parameter, the directory where the simulations will be run (if omitted, a temporary directory is created automatically). The `DSSAT.run` method has been replaced with the `DSSAT.run_treatment` method. 
...
1. Create a new DSSAT instance. When DSSAT is instantiated, a simulation environment is set. The `run_path` argument is **optional** — pass a directory to set the environment there, or omit it (`DSSAT()`) to have a temporary directory created automatically:
   ```python
    >>> dssat = DSSAT("/tmp/dssat_test")   # explicit path
    >>> dssat = DSSAT()                     # auto tmp directory
   ```
