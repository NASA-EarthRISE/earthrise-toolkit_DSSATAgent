...
def _get_croppars(spe_path, code, dtypes_dict, pars_fmt_dict, par_prefix):
    ...
    class CropPars(Record):
        ...
        def __init__(self):
            ...
            # Detect header character from file content rather than
            # hardcoding crop codes — some newer models (BACRP, etc.)
            # also use '$'.
            header_character = '*'
...
class Crop(MutableMapping):
    ...
    def cultivar_list(cls):
        ...
        encoding = detect_encoding(cul_path)
        with open(cul_path, "r", encoding=encoding) as f:
            lines = f.readlines()
        # Strip BOM then filter comment/header lines
