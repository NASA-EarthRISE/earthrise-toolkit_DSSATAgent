...
def estimate_from_texture(slcl, slsi, sbdm=None, sloc=None):
    ...
    if sbdm:
        soil_input = [[100 - slcl - slsi, slsi, slcl, sbdm]]
        model_code = 3
    else:
        soil_input = [[100 - slcl - slsi, slsi, slcl]]
        model_code = 2

    if SoilData is not None and hasattr(SoilData, 'from_array'):
        soil_input = SoilData.from_array(soil_input)
    vangenuchten_pars, _, _ = rosetta(model_code, soil_input)
        
    vangenuchten_pars = vangenuchten_pars[0]
    ssat = vangenuchten_pars[1]
    ssks = (10**vangenuchten_pars[4]) / 24
    slll = van_genuchten(*vangenuchten_pars[:4], h=1500)
    sdul = van_genuchten(*vangenuchten_pars[:4], h=33)
...
class SoilProfile(TabularRecord):
    ...
    @property
    def str(self):
        # DSSAT FILEX/SOIL.SOL convention: the soil-profile name is a
        # fixed 10-char column. ``DescriptionType.__new__`` strips
        # trailing whitespace at construction, so a 9-char ID like
        # ``AL1147544`` ends up un-padded. Without re-padding here, the
        # FILEX FIELDS row's ID_SOIL ends one column short of where the
        # SOIL.SOL header sits, IPSOIL mis-parses subsequent layer rows
        # (each row's depth column shifts), and the parser eventually
        # walks past the section delimiter — surfacing as ``IPSOIL: More
        # than 19 layers in the soil profile`` (error 5010) at run time.
        name = str(self['name'])
        return name.ljust(10) if len(name) < 10 else name[:10]
